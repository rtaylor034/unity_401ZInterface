...
public class TESTER : MonoBehaviour
{
    ...
    async void Start()
    {
        ...
        var token_complicated = MakeToken.tRecursive<r.Number, r.Multi<r.Number>, r.Number>(new() // if you can figure out what this does, then you understand the language. (yes, there is recursion)
        {
            A = 0.tConst(),
    ...
    async void Update()
    {
        ...
        {
            A = 0.tConst(),
            B = 0.Sequence(x => x + 1).Take(5).Map(x => x.tConst()).tToMulti(),
            RecursiveProxyStatement = P =>
                P.pSubEnvironment(RHint<r.Number>.Specify(), new()
                {
                    EnvironmentProxies = new()
                    ...
                    SubProxy = P.pOriginalA().pIsGreaterThan(2.tConst().pAsProxyFor(P)).pIfTrue(RHint<r.Number>.Specify(), new()
                    {
                        Then = counter.tRef().pAsProxyFor(P),
                        Else = P.pRecurseWith(new()
                        {
                            A = counter.tRef().tAdd(1.tConst()).pAsProxyFor(P),
                            B = pool.tRef().tWithout(counter.tRef().tYield()).pAsProxyFor(P)
                        }).pAdd(counter.tRef().pAsProxyFor(P))
                    })
                })
        });
        await token1.ResolveWithRules(_program);
        //Debug.Log(await token1.ResolveWithRules(_program));
        */
    }
}
